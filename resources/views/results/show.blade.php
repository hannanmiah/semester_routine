    ...
    <div class="mt-10 sm:mt-0">
        <div class="md:grid md:grid-cols-3 md:gap-6">
            <div class="mt-5 md:col-span-3 md:mt-0">
                <form action="{{route('results.update',['result' => $result])}}" method="POST">
                    @method('PUT')
                    @csrf
                            ...
                            <div class="flex flex-col space-y-2 md:space-y-4">
                                <div class="flex flex-col space-y-1 md:space-y-2">
                                    <label for="first-name" class="block text-sm font-medium text-gray-700">Course
                                        Code</label>
                                    <input type="text" name="course_code" id="first-name"
                                           value="{{$result->course_code}}"
                                ...
                                <div class="flex flex-col space-y-1 md:space-y-2">
                                    ...
                                    <input type="text" name="course_name" id="email-address"
                                           value="{{$result->course_name}}" autocomplete="course_name"
                                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                    @error('course_name')
                                    <span class="text-red-500">{{$message}}</span>
                                    @enderror
